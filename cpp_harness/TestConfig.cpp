...
/*
// this is super wrong
	for(int i = 0; i<task_num; i+=2){
		affinities[i]=i*2;
	}
	for(int i = 1; i<task_num; i+=2){
		affinities[i]=(i-num_procs/2)*2+1;
	}
*/
	int c = 0;
	for(int i = 0; i<task_num; i++){
...
void GlobalTestConfig::buildEvenOddLoHiAffinity(){
	...
	for(int i = 0; i<task_num; i++){
		...
		else if(i<num_procs/2){
			c=(i-num_procs/4)*2+1;
		}
		else if(i<3*num_procs/4){
			c=(i-num_procs/2)*2+num_procs/2;
		}
		else{
			c=(i-num_procs/2)*2+1;
		}
